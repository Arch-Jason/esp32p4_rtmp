...
int flv_tag_header_read(struct flv_tag_header_t* tag, const uint8_t* buf, size_t len) {
    ...
    assert(FLV_TYPE_VIDEO == tag->type || FLV_TYPE_AUDIO == tag->type
           || FLV_TYPE_SCRIPT == tag->type);
...
static int flv_audio_tag_header_read_fourcc(struct flv_audio_tag_header_t* audio,
                                            const uint8_t* buf,
                                            size_t len) {
    if (len < 4)
        return -1;
    ...
    if (FLV_AUDIO_AAC == audio->codecid || FLV_AUDIO_OPUS == audio->codecid) {
        ...
        if (audio->avpacket == FLV_AUDIO_PACKET_TYPE_MULTICHANNEL_CONFIG) {
            ...
            if (1 == audio->channelorder) {
                ...
                audio->channelflags = (((uint32_t) buf[off + 0]) << 24)
                ...
                memcpy(audio->channelmapping,
                       buf,
                       audio->channels
                               <= sizeof(audio->channelmapping) / sizeof(audio->channelmapping[0])
                           ? audio->channels
                           : (sizeof(audio->channelmapping) / sizeof(audio->channelmapping[0])));
                off += audio->channels;
            }
...
int flv_video_tag_header_read(struct flv_video_tag_header_t* video, const uint8_t* buf, size_t len) {
    ...
    if (len >= 5 && 0 != (buf[0] & 0x80)) {
        ...
        case FLV_VIDEO_FOURCC_VVC:
            video->codecid = (FLV_VIDEO_FOURCC(buf[1], buf[2], buf[3], buf[4])
                              == FLV_VIDEO_FOURCC_HEVC)
                                 ? FLV_VIDEO_H265
                                 : FLV_VIDEO_H266;
    ...
    if (FLV_VIDEO_H264 == video->codecid || FLV_VIDEO_H265 == video->codecid
        || FLV_VIDEO_H266 == video->codecid || FLV_VIDEO_AV1 == video->codecid
        || FLV_VIDEO_AVS3 == video->codecid) {
        if (len < 5)
            return -1;
        ...
        //if (video->cts >= (1 << 23)) video->cts -= (1 << 24);
        video->cts = (video->cts + 0xFF800000) ^ 0xFF800000; // signed 24-integer
        assert(FLV_SEQUENCE_HEADER == video->avpacket || FLV_AVPACKET == video->avpacket
               || FLV_END_OF_SEQUENCE == video->avpacket);
        return 5;
    } else {
        return 1;
    }
...
int flv_audio_tag_header_write(const struct flv_audio_tag_header_t* audio,
                               uint8_t* buf,
                               size_t len) {
    ...
    if (FLV_AUDIO_AAC == audio->codecid) {
        assert(FLV_SEQUENCE_HEADER == audio->avpacket || FLV_AVPACKET == audio->avpacket);
        buf[0] = (audio->codecid) | (audio->rate << 2) | (audio->bits << 1) | (audio->channels);
        buf[1] = audio->avpacket; // AACPacketType
        return 2;
    } else if (FLV_AUDIO_OPUS == audio->codecid || FLV_AUDIO_FLAC == audio->codecid
               || FLV_AUDIO_AC3 == audio->codecid || FLV_AUDIO_EAC3 == audio->codecid) {
        if (len < 5)
            return -1;
        ...
        buf[0] = (audio->codecid /* <<4 */) | ((audio->rate & 0x03) << 2)
                 | ((audio->bits & 0x01) << 1) | (audio->channels & 0x01);
        return 1;
    }
}

int flv_video_tag_header_write(const struct flv_video_tag_header_t* video,
                               uint8_t* buf,
                               size_t len) {
    // https://github.com/veovera/enhanced-rtmp/blob/main/enhanced-rtmp.pdf
    if (video->enhanced_rtmp) {
        ...
        buf[0] |= (0 == video->cts && FLV_AVPACKET == video->avpacket)
